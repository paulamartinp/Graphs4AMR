# 2018/11/01. The basic GSP and graph ML functions to create GNNs.
"""
...
HybridEdgeVariantGF: Creates a graph filtering layer using hybrid edge-variant
    graph filters
GraphAttentional: Creates a linear layer using graph attention mechanisms
...
def NLSIGF(h, S, x, nonlinearity, b=None, leak=None):
    """
    """
    # The basic idea of what follows is to start reshaping the input and the
    # GSO so the filter coefficients go just as a very plain and simple
    ...
    for k in range(1,K):
        ...
        if leak is None:
            ...
            xS = nonlinearity(xS,negative_slope=leak)
        z = torch.cat((z, xS), dim = 2) # B x E x k x G x N
    # This output z is of size B x E x K x G x N
    # Now we have the x*S_{e}^{k} product, and we need to multiply with the
    # filter taps.
...
class NLGraphFilter(nn.Module):
    """
    """

    def __init__(self, G, F, K, nonlinearity, E = 1, leak=None, bias = True):
        # K: Number of filter taps
        # GSOs will be added later.
        # This combines both weight scalars and weight vectors.
        # Bias will always be shared and scalar.
    ...
    def forward(self, x):
        ...
        # Compute the filter output
        u = NLSIGF(self.weight, self.S, x, self.nonlinearity, self.bias, self.leak)
        # So far, u is of shape batchSize x dimOutFeatures x numberNodes
        # And we want to return a tensor of shape
        # batchSize x dimOutFeatures x numberNodesIn
        # since the nodes between numberNodesIn and numberNodes are not required
...
class RSNLayer(nn.Module):
    """
    RSNLayer Creates a (linear) layer that applies an RSN of order one

    Initialization:

        GraphFilter(in_features, state_features, out_features, filter_taps, 
                    state_nonlinearity, output_nonlinearity, edge_features=1, bias=True)
        ...
        Inputs:
            ...
            out_features (int): number of output features (each feature is a
                graph signal)
            filter_taps (int): number of filter taps
            state_nonlinearity (nn.functional): state activation function
            output_nonlinearity (nn.functional): output activation function
            edge_features (int): number of features over each edge
            bias (bool): add bias vector (one bias per feature) after graph
                filtering

        Output:
            torch.nn.Module for an order one RSN layer 
    ...
    Forward call:
        ...
        Inputs:
            x (torch.tensor): input data; shape:
                batch_size x in_features x number_nodes
            w0 (torch.tensor): initial state; shape:
                batch_size x state_features x number_nodes

        Outputs:
            y (torch.tensor): output; shape:
                batch_size x out_features x number_nodes
            wN (torch.tensor): last state; shape:
                batch_size x state_features x number_nodes
    """

    def __init__(self, G, H, F, K, state_nonlinearity, output_nonlinearity, bias = True, E = 1,):
        # K: Number of filter taps
        # GSOs will be added later.
        # This combines both weight scalars and weight vectors.
        # Bias will always be shared and scalar.
        ...
        self.E = E
        self.H = H
        self.sigma1 = state_nonlinearity
        self.sigma2 = output_nonlinearity
        self.S = None # No GSO assigned yet
        # Create parameters:
        ...
        for i in range(self.K):
            self.weightsW.append(nn.parameter.Parameter(torch.Tensor(H, H)))
            self.weightsX.append(nn.parameter.Parameter(torch.Tensor(H, G)))
            self.weightsY.append(nn.parameter.Parameter(torch.Tensor(F, H)))
        self.finalWeight = nn.parameter.Parameter(torch.Tensor(1,K))
        self.bias = bias
        if self.bias:
            self.biasW = nn.parameter.Parameter(torch.Tensor(H, 1))
            self.biasY = nn.parameter.Parameter(torch.Tensor(F, 1))
            self.biasFinalY = nn.parameter.Parameter(torch.Tensor(F, 1))
        # Initialize parameters
        self.reset_parameters()

    def reset_parameters(self):
        # Taken from _ConvNd initialization of parameters:
        stdvX = 1. / math.sqrt(self.G)
        stdvW = 1. / math.sqrt(self.H)
        stdvY = 1. / math.sqrt(self.H)
        stdvFinalY = 1. / math.sqrt(self.K)
        for i in range(self.K):
            self.weightsX[i].data.uniform_(-stdvX, stdvX)
            self.weightsW[i].data.uniform_(-stdvW, stdvW)
            self.weightsY[i].data.uniform_(-stdvY, stdvY)
        self.finalWeight.data.uniform_(-stdvFinalY, stdvFinalY)
        if self.bias:
            self.biasW.data.uniform_(-stdvW, stdvW)
            self.biasY.data.uniform_(-stdvY, stdvY)
            self.biasFinalY.data.uniform_(-stdvFinalY, stdvFinalY)
...
#        N = x.shape[2]
        w = w0
        Y = torch.empty(0)
        # Compute the RSN states
        for i in range(self.K):
            wS = torch.matmul(w, S) # B x E x G x N
            wS = w.reshape([B, 1, G, N]) # B x E x 1 x G x N
            
            S=self.S.reshape(1,self.N,self.N)
            S=S.repeat(B,1,1)
            Sw = torch.matmul(S,w.transpose(2,1))
            w = torch.matmul(self.weightsW[i],Sw.transpose(2,1))
            w = w + torch.matmul(self.weightsX[i],x)
#            if self.bias:
#                w = w + self.biasW
            w = self.sigma1(w)
            y = torch.matmul(self.weightsY[i],w)
#            if self.bias:
#                y = y + self.biasY
            #y=self.sigma2(y)
            y = y.unsqueeze(0)
        ...
        Y=Y.transpose(0,1)
        finalY = torch.matmul(self.finalWeight,Y.transpose(1,2)).squeeze()
        if self.bias:
            finalY = finalY + self.biasFinalY
        finalY=self.sigma2(finalY)
        return finalY, w
...
class NodeVariantGF(nn.Module):
    ...
    def addGSO(self, S):
        ...
        if self.M < self.N:
            ...
            self.copyNodes = torch.tensor(np.asarray(copyNodes)).to(S.device)
        elif self.M == self.N:
    ...
    def forward(self, x):
        ...
        if self.M == self.N:
            ...
            self.h = torch.index_select(self.weight, 4, self.copyNodes.long())
        # And now we add the zero padding
        if Nin < self.N:
            zeroPad = torch.zeros(B, F, self.N-Nin).type(x.dtype).to(x.device)
            x = torch.cat((x, zeroPad), dim = 2)
...
class GGCRNNCell(nn.Module):
    ...
    def __init__(self, G, F, Kin, Kst, sigma=nn.Tanh, time_gating=True, spatial_gating=None, E=1, bias=True):
        ...
        print("weight_A:", self.weight_A.shape)
        print("weight_B:", self.weight_B.shape)
        self.sigma = sigma
        self.time_gating = time_gating
        self.spatial_gating = spatial_gating
        print("time_gating:", self.time_gating)
        print("spatial_gating:", self.spatial_gating)
        self.bias_flag = bias
        ...
        for i in range(T):
            ...
            if self.spatial_gating is not None:
                ...
                if self.spatial_gating == 'node':
                    ...
                    # Apply filters to input and state
                    h = in_gate.view(-1, 1, 1).repeat(1, self.F, self.N) * self.input_attention(
                        LSIGF(self.weight_A, self.S, x, self.bias)) \
                        + forget_gate.view(-1, 1, 1).repeat(1, self.F, self.N) * self.forget_attention(
                        LSIGF(self.weight_B, self.S, h, self.bias))
                    # Apply hyperbolic tangent
                    h = self.sigma(h)
            else:
