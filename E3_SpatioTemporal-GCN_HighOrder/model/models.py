...
class STGCNHighOrderGraphConv(nn.Module):
    """
    STGCN with High-Order Graph Convolutions
    """

    def __init__(self, args, blocks, n_vertex):
        super(STGCNHighOrderGraphConv, self).__init__()
        modules = []
        for l in range(len(blocks) - 3):
            modules.append(layers.STConvBlock(
                args.Kt, args.Ks, n_vertex, blocks[l][-1], blocks[l+1], 
                args.act_func, 'high_order_graph_conv', args.gso, args.enable_bias, args.droprate
            ))
